...
def end():
    ...
    for i in data.keys():
        for j in range(len(clean_data[0])):
...
'''LABELS'''
meme_name = Label(game_screen, text=meme["name"], fg=name_meme_set["color"], font=name_meme_set["font"],
                  height=name_meme_set["height"], width=name_meme_set["width"])
meme_text = Label(game_screen, text=meme["text"], fg=text_meme_set["color"], font=text_meme_set["font"],
                  height=text_meme_set["height"], width=text_meme_set["width"])
result_top_text = Label(result_screen, text=result_text_set["text"], fg=result_text_set["color"],
                        font=result_text_set["font"], height=result_text_set["height"], width=result_text_set["width"])
result_text = Label(result_screen, fg=result_text_set["color"], font=result_text_set["font_val"],
                    height=result_text_set["height"], width=result_text_set["width"])
...
# menu
quitButton = Button(menu, bg=quitButton["bg"], fg=quitButton["fg"], text=quitButton["text"], command=menu_quit,
                    font=quitButton["font"])
infoButton = Button(menu, bg=infoButton["bg"], fg=infoButton["fg"], text=infoButton["text"], command=menu_info,
                    font=infoButton["font"])
startButton = Button(menu, bg=startButton["bg"], fg=startButton["fg"], text=startButton["text"], command=menu_start,
                     font=startButton["font"])

# main
like = Button(buttons, height=button_set["height"], width=button_set["width"], text=button_set["like"],
              bg=button_set["like_color"], font=button_set["font"], command=lambda: upd(meme["name"], meme["id"], True))
dislike = Button(buttons, height=button_set["height"], width=button_set["width"], text=button_set["dislike"],
                 bg=button_set["dislike_color"], font=button_set["font"],
                 command=lambda: upd(meme["name"], meme["id"], False))
